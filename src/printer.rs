...
use crate::rules::variable::{self, VariableDefined};
use crate::rules::{Location, StyleContext};
use crate::selector::{Component, Selector, SelectorList};
use crate::targets::Targets;
use crate::values::ident::Ident;
use crate::values::string::CowArcStr;
use crate::vendor_prefix::VendorPrefix;
use cssparser::{serialize_identifier, serialize_name};
use parcel_selectors::builder::{SelectorFlags, SpecificityAndFlags};
...
pub struct Printer<'a, 'b, 'c, W> {
  ...
  selector_context: Vec<Vec<Selector<'b>>>,
}
...
impl<'a, 'b, 'c, W: std::fmt::Write + Sized> Printer<'a, 'b, 'c, W> {
  ...
  pub(crate) fn with_selector_context_inner(
    &self,
    parent_selectors: &Vec<Selector<'b>>,
    selectors: &SelectorList<'b>,
  ) -> Result<Vec<Selector<'b>>, ()> {
    let mut current_level = vec![];
    for selector in selectors.0.iter() {
      let len = if parent_selectors.len() == 0 {
        1
      } else {
        parent_selectors.len()
      };
      let nest_count = selector.1.iter().filter(|c| matches!(c, Component::Nesting)).count();
      let loop_time = len.pow(nest_count as u32);
      let mut nest_array = vec![0usize; nest_count];
      let max_length = parent_selectors.len();
      let mut components = vec![];
      let mut si = selector.iter_raw_match_order().rev();
      'out: loop {
        let mut m = vec![];
        'inner: loop {
      ...
      for _ in 0..loop_time {
        ...
        for (index, &component) in components.iter().enumerate() {
          dbg!(&nest_array, current_nest, index);

          match component {
            Component::Nesting => {
              if let Some(&cur_index) = nest_array.get(current_nest) {
                if let Some(parent_selector) = parent_selectors.get(cur_index) {
                  for component in &parent_selector.1 {
                    new_selector.1.push(component.clone());
                  }
                }
              }
              current_nest += 1;
            }
            _ => new_selector.1.push(component.clone()),
          }
        }
        dbg!(&new_selector.1);
        if max_length != 0 {
          next(&mut nest_array, max_length);
        }
        current_level.push(new_selector);
      }
    }
    dbg!(&current_level);
  ...
  pub(crate) fn with_selector_context<T, U, F: FnOnce(&mut Printer<'a, 'b, 'c, W>) -> Result<T, U>>(
    &mut self,
    selectors: Option<&SelectorList>,
    f: F,
  ) -> Result<T, U> {
    ...
    if let Some(selectors) = selectors {
      if let Some(before_level) = self.selector_context.last() {
        current_level = self
          .with_selector_context_inner(before_level, unsafe { std::mem::transmute(selectors) })
          .unwrap();
      } else {
        current_level = self
          .with_selector_context_inner(&vec![], unsafe { std::mem::transmute(selectors) })
          .unwrap();
      }
    }

    self.selector_context.push(current_level);
    let res = f(self);
    self.selector_context.pop();
    res
  }

  pub fn get_selector_context(&self) -> Option<Vec<Selector<'b>>> {
    dbg!(&self.selector_context);
    match self.selector_context.len() {
      0 => None,
      n => Some(self.selector_context.last().unwrap().clone()),
    }
  }
...
fn selector_context() {
  ...
  let mut p = Printer::new(&mut dest, Default::default());
  let selector = parcel_selectors::SelectorList(smallvec![
    parcel_selectors::parser::Selector(
      SpecificityAndFlags {
        specificity: 1,
  ...
  let a = parcel_selectors::SelectorList(smallvec![
    parcel_selectors::parser::Selector(
      SpecificityAndFlags {
        specificity: 3,
        flags: SelectorFlags::empty()
      },
      vec![Component::Nesting, Component::ID(Ident("c".into())), Component::Nesting,]
    ),
    parcel_selectors::parser::Selector(
      SpecificityAndFlags {
        specificity: 4,
  ...
  let b = parcel_selectors::SelectorList(smallvec![
    parcel_selectors::parser::Selector(
      SpecificityAndFlags {
        specificity: 5,
        flags: SelectorFlags::empty()
  ...
  let _ = p
    .with_selector_context(Some(&selector), |p| {
      dbg!(&p.selector_context);
      let _ = p
        .with_selector_context(Some(&a), |p| {
          dbg!(&p.selector_context);

          let _ = p
            .with_selector_context(Some(&b), |p| {
              dbg!(&p.selector_context);
...
fn next(nest_array: &mut Vec<usize>, max: usize) {
  let mut cur_index = nest_array.len() - 1;
  let mut is_forward = false;
  loop {
    nest_array.get_mut(cur_index).map(|mut current| {
      *current = *current + 1usize;
      if *current == max {
        *current = 0;
        is_forward = true;
      } else {
