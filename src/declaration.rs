...
use crate::properties::custom::{CustomPropertyName, TokenList, TokenOrValue};
...
use cssparser::*;
use crate::properties::custom::TokenOrValue::Token;
...
impl<'a, 'o, 'i> VariableDefineParser<'i> for PropertyDeclarationParser<'a, 'o, 'i> {
    type Error = ParserError<'i>;
    type Value = ();
    fn parse_value_defined<'t>(&mut self, start: &ParserState, name: CowRcStr<'i>, input: &mut Parser<'i, 't>) -> Result<Self::Value, ParseError<'i, Self::Error>> {
...
pub(crate) fn parse_declaration<'i, 't>(
    ...
    variables: &mut Vec<VariableDefined<'i>>,
    options: &ParserOptions<'_, 'i>,
) -> Result<(), cssparser::ParseError<'i, ParserError<'i>>> {
    //FIXME: 属性没有变量
    if input.try_parse(|dest| dest.expect_colon()).is_ok() {
        variables.push(parse_variable_defined(name.clone(), input, options)?);
    }
