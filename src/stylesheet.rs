...
use cssparser::{CowRcStr, Parser, ParserInput, StyleSheetParser};
        ...
        T: ToCss + Clone,
{
    ...
    pub fn new(
        ...
        variables: Vec<VariableDefined<'i>>,
        options: ParserOptions<'o, 'i>,
    ) -> StyleSheet<'i, 'o, T> {
        StyleSheet {
            sources,
    ...
    pub fn parse_with<P: AtRuleParser<'i, AtRule=T>>(
        ...
        let mut rule_parser = TopLevelRuleParser::new(&mut options, at_rule_parser, &mut rules, &mut variables);
    ...
    pub fn to_css(&self, options: PrinterOptions) -> Result<ToCssResult, Error<PrinterErrorKind>> {
        ...
        if let Some(config) = &self.options.css_modules {
            ...
            printer.with_context(None, Some(&self.variables), |dest| {
