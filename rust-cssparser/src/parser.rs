...
pub enum BasicParseErrorKind<'i> {
  ...
  /// An unexpected variable defined was encountered
...
impl<'i> fmt::Display for BasicParseErrorKind<'i> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      BasicParseErrorKind::VariableDefinedInvalid => {
        write!(f, "invalid variable defined encountered")
      }
    }
  }
}
...
impl<'i: 't, 't> Parser<'i, 't> {
  ...
  pub fn next_including_whitespace_and_comments(&mut self) -> Result<&Token<'i>, BasicParseError<'i>> {
    ...
    let using_cached_token = self.input.cached_token.as_ref().map_or(false, |cached_token| {
