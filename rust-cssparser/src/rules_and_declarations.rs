...
/// variable define parser
pub trait VariableDefineParser<'i> {
  /// todo
  type Error: 'i;
  ...
  fn parse_value_defined<'t>(
    ...
    Err(input.new_error(BasicParseErrorKind::VariableDefinedInvalid))
  }
}
...
pub trait AtRuleParser<'i> {
  ...
  fn rule_without_block<'t>(&mut self, prelude: Self::Prelude, start: &ParserState) -> Result<Self::AtRule, ()> {
    let _ = prelude;
    let _ = start;
    Err(())
  }
...
pub trait RuleBodyItemParser<'i, DeclOrRule, Error: 'i>:
  ...
  + VariableDefineParser<'i, Value = DeclOrRule, Error = Error>
{
  ...
  fn next(&mut self) -> Option<Self::Item> {
    loop {
      ...
      match self.input.next_including_whitespace_and_comments().ok()? {
        Token::CloseCurlyBracket | Token::WhiteSpace(..) | Token::Semicolon | Token::Comment(..) => continue,
        Token::AtKeyword(ref name) => {
          let name = name.clone();
          if self.input.try_parse(|dest| dest.expect_colon()).is_ok() {
            return Some(parse_variable_define(&start, name, self.input, &mut *self.parser));
          }
        ...
        Token::Ident(ref name) if self.parser.parse_declarations() => {
          ...
          let result = {
            ...
            parse_until_after(self.input, Delimiter::Semicolon, error_behavior, |input| {
              input.expect_colon()?;
...
where
  P: QualifiedRuleParser<'i, QualifiedRule = R, Error = E>
    ...
    + VariableDefineParser<'i, Error = E>,
{
...
where
  P: QualifiedRuleParser<'i, QualifiedRule = R, Error = E>
    + AtRuleParser<'i, AtRule = R, Error = E>
    + VariableDefineParser<'i, Value = R, Error = E>,
{
  ...
  fn next(&mut self) -> Option<Self::Item> {
    loop {
      ...
      if let Some(name) = at_keyword {
        ...
        self.any_rule_so_far = true;
        let result = parse_qualified_rule(&start, self.input, &mut *self.parser, Delimiter::CurlyBracketBlock);
        return Some(result.map_err(|e| (e, self.input.slice_from(start.position()))));
      }
    }
  }
...
pub fn parse_one_rule<'i, 't, R, P, E>(input: &mut Parser<'i, 't>, parser: &mut P) -> Result<R, ParseError<'i, E>>
where
...
fn parse_variable_define<'i, 't, P, E>(
  start: &ParserState,
  name: CowRcStr<'i>,
  input: &mut Parser<'i, 't>,
  parser: &mut P,
) -> Result<<P as VariableDefineParser<'i>>::Value, (ParseError<'i, E>, &'i str)>
where
  P: VariableDefineParser<'i, Error = E>,
{
  parser
    .parse_value_defined(start, name, input)
